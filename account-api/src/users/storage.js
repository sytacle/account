...
function planFeatures(subscription) {
  const plans = config.subscription.plans;
  return (plans[subscription] || plans.free).features;
}

function storageLimit(subscription) {
  return planFeatures(subscription).storage.limit;
}

function assertContentTypeAllowed(subscription, contentType) {
  const access = planFeatures(subscription).storage.access;
  ...
  if (!access[category])
    throw new ApiError(
      "plan_restricted",
      `Your plan does not allow uploading ${category}.`,
      403,
    );
}
...
async function assertUploadWithinLimit(uid, subscription, size) {
  ...
  if (totalBytes + size > storageLimit(subscription))
    throw new ApiError(
      ...
      "Your plan storage limit has been reached.",
      413,
    );
}
...
function validateFile(body) {
  ...
  if (
    ...
    throw new ApiError(
      "invalid_request",
      "A valid file name, type, and size are required.",
      400,
    );
...
export async function listFiles(req, res) {
  ...
  const [snapshot, summary] = await Promise.all([
    filesRef(token.uid).orderBy("createdAt", "desc").get(),
    summaryRef(token.uid).get(),
  ]);
  const data = summary.exists ? summary.data() : {};
...
export async function storageSummary(req, res) {
  const token = await verifyBearerToken(req);
  const [snapshot, summary] = await Promise.all([
    filesRef(token.uid).orderBy("createdAt", "desc").get(),
    summaryRef(token.uid).get(),
    ...
    totalBytes: Number(
      data.totalBytes ||
        snapshot.docs.reduce(
          (total, doc) => total + Number(doc.data().size || 0),
          0,
        ),
    ),
  });
...
export async function createUploadUrl(req, res) {
  ...
  assertContentTypeAllowed(token.subscription, file.contentType);
...
export async function completeUpload(req, res) {
  ...
  if (!id || !key.startsWith(`${token.uid}/`))
    throw new ApiError("invalid_request", "Invalid upload reference.", 400);
  assertContentTypeAllowed(token.subscription, file.contentType);
  const object = await r2().send(
    new HeadObjectCommand({ Bucket: process.env.R2_BUCKET, Key: key }),
  );
  if (
    Number(object.ContentLength || 0) !== file.size ||
    object.ContentType !== file.contentType
  )
    throw new ApiError(
      "invalid_request",
      "Uploaded file metadata does not match the object.",
      400,
    );
...
export async function deleteFile(req, res) {
  ...
  if (data.provider === "cloudinary") {
    ...
    await r2().send(
      new DeleteObjectCommand({ Bucket: process.env.R2_BUCKET, Key: data.key }),
    );
  }
...
export async function completeCloudinaryProfileImage(req, res) {
  const token = await verifyBearerToken(req);
  const url =
    typeof req.body?.secureUrl === "string" ? req.body.secureUrl.trim() : "";
  const publicId =
    typeof req.body?.publicId === "string" ? req.body.publicId.trim() : "";

  if (!url || !publicId || !url.startsWith("https://res.cloudinary.com/"))
    throw new ApiError("invalid_request", "Invalid Cloudinary image.", 400);
  ...
  await assertUploadWithinLimit(
    token.uid,
    token.subscription,
    Math.max(0, size - previousSize),
  );
  await ref.set(
    {
      name: "Profile image",
...
async function authUpdateProfile(uid, photoURL) {
  ...
  await db
    ...
    .doc(uid)
    .set(
      { photoURL, updatedAt: FieldValue.serverTimestamp() },
      { merge: true },
    );
}
