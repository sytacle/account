...
function ProfileCard() {
  ...
  async function handleSave(e) {
    ...
    setError("");
    try {
      await saveProfile({ displayName: name.trim(), phone: phone.trim(), birthday });
      setEditing(false);
    } catch (err) {
      ...
      <div className="p-5 sm:p-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          <div className="grid size-20 shrink-0 place-items-center overflow-hidden rounded-full bg-blue-600 text-2xl font-semibold text-white">
            {user?.photoURL ? (
              <img src={user.photoURL} alt="" className="size-20 object-cover" />
            ) : (
          ...
          <div className="min-w-0 flex-1">
            ...
            {user?.emailVerified ? (
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                <button onClick={handleResend} className="text-xs font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400">
                  Resend email
                </button>
              </div>
            )}
            {verifyNotice && <p className="mt-1.5 text-xs text-slate-500 dark:text-slate-400">{verifyNotice}</p>}
          </div>
          {!editing && (
            <button
        ...
        {editing ? (
          <form onSubmit={handleSave} className="mt-6 space-y-4 border-t border-slate-100 pt-5 dark:border-slate-800">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400">Phone number</label>
                <input
                  value={phone}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400">Birthday</label>
                <input
                  type="date"
                  value={birthday}
          ...
          <div className="mt-6 grid border-t border-slate-100 pt-5 dark:border-slate-800 sm:grid-cols-2 lg:grid-cols-4">
            <Info icon={UserRound} label="Full name" value={user?.displayName || "Not set"} />
            <Info icon={Mail} label="Email address" value={user?.email} />
            <Info icon={Phone} label="Phone number" value={profile?.phone || "Not set"} />
            <Info icon={CalendarDays} label="Birthday" value={profile?.birthday || "Not set"} />
          </div>
        )}
      </div>
    </Card>
...
function SecurityCard() {
  ...
  return (
    <Card>
      <CardTitle title="Account security" subtitle="Keep your account safe and secure." />
      <div className="divide-y divide-slate-100 dark:divide-slate-800">
        ...
        <Row
          ...
          description={user?.emailVerified ? "Your email is verified" : "Verify your email to secure recovery"}
          value={user?.emailVerified ? "Verified" : "Pending"}
        />
      </div>
      <div className="border-t border-slate-100 px-5 py-3 dark:border-slate-800">
        <Link to="/account/security" className="text-sm font-medium text-blue-700 dark:text-blue-400">
          Manage security →
        </Link>
      </div>
    </Card>
...
function LinkedCard() {
  ...
  return (
    <Card>
      <CardTitle title="Linked accounts" subtitle="Connect your accounts for a better experience." />
      <div className="divide-y divide-slate-100 dark:divide-slate-800">
        {oauthProviders.map((item) => {
          const providerInfo = user?.providerData?.find((p) => p.providerId === item.id);
            ...
            <div key={item.id} className="flex items-center gap-3 px-5 py-3.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-slate-500 dark:text-slate-400">
                  {connected ? providerInfo?.email || "Connected" : "Not connected"}
                </p>
              </div>
              {connected ? (
...
function SmallCard({ title, text, to }) {
  return (
    <Card className="p-5">
      ...
      <Link to={to} className="mt-4 inline-block text-sm font-medium text-blue-700 dark:text-blue-400">
        Manage →
      </Link>
    </Card>
  );
