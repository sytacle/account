...
function BillingPage() {
  ...
  const [accounts, setAccounts] = useState([]); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(""); const [error, setError] = useState(""); const [notice, setNotice] = useState("");
  async function load() { try { const { getBilling } = await import("../lib/accountApi.js"); setAccounts((await getBilling(user)).accounts || []); } catch (err) { setError(friendlyAuthError(err)); } finally { setLoading(false); } }
  useEffect(() => { load(); }, [user]);
  async function activate(id) { setBusy(id); setError(""); try { const { activateBillingAccount } = await import("../lib/accountApi.js"); await activateBillingAccount(user, id); setAccounts((items) => items.map((account) => ({ ...account, isActive: account.id === id }))); setNotice("Current billing account updated."); } catch (err) { setError(friendlyAuthError(err)); } finally { setBusy(""); } }
  return <Page back icon={WalletCards} title="Billing accounts" subtitle="Choose an available account for invoices and receipts."><Message error={error} notice={notice} />{loading ? <Loading /> : <Card><div className="flex items-center justify-between border-b border-slate-100 px-5 py-5 dark:border-slate-800"><div><h3 className="font-semibold text-slate-900 dark:text-slate-100">Available accounts</h3><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Only saved billing accounts appear here.</p></div><button type="button" onClick={() => navigate("/account/payments/billing/new")} className="rounded-full bg-blue-600 px-3 py-2 text-xs font-medium text-white">Add account</button></div>{accounts.length ? accounts.map((account) => <div key={account.id} className="flex items-center gap-3 border-b border-slate-100 px-5 py-4 last:border-0 dark:border-slate-800"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-900 dark:text-slate-100">{account.name || "Billing account"}</p><p className="truncate text-xs text-slate-500 dark:text-slate-400">{account.email || "No billing email"}</p></div>{account.isActive && <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400">Current</span>}<button type="button" onClick={() => navigate(`/account/payments/billing/${account.id}`)} className="text-xs font-medium text-blue-600 dark:text-blue-400">Manage</button>{!account.isActive && <button type="button" onClick={() => activate(account.id)} disabled={busy === account.id} className="text-xs font-medium text-slate-600 disabled:opacity-60 dark:text-slate-300">{busy === account.id ? "Switching..." : "Make current"}</button>}</div>) : <div className="px-5 py-8 text-sm text-slate-500 dark:text-slate-400">No billing accounts yet. Create one to use for subscriptions and receipts.</div>}</Card>}</Page>;
}
...
function SubscriptionsPage() {
  const { user } = useAuth(); const [planKey, setPlanKey] = useState("free"); const [subscriptions, setSubscriptions] = useState([]); const [products, setProducts] = useState([]); const [error, setError] = useState(""); const [busy, setBusy] = useState(""); const [notice, setNotice] = useState("");
  async function load() { const api = await import("../lib/accountApi.js"); const [tokenResult, subscriptionResult, productResult] = await Promise.allSettled([user.getIdTokenResult(true), api.getSubscriptions(user), api.getSubscriptionProducts(user)]); if (tokenResult.status === "fulfilled") { const claimedPlan = tokenResult.value.claims?.subscription || "free"; setPlanKey(subscriptionPlans[claimedPlan] ? claimedPlan : "free"); } else setError(friendlyAuthError(tokenResult.reason)); if (subscriptionResult.status === "fulfilled") setSubscriptions(subscriptionResult.value.subscriptions || []); else setError(friendlyAuthError(subscriptionResult.reason)); if (productResult.status === "fulfilled") setProducts(productResult.value.products || []); else setError(friendlyAuthError(productResult.reason)); }
  useEffect(() => { load(); }, [user]);
